...
class Daftar_sbm extends CI_Controller {
	private $data;
	...
	public function list(){
		...
		if(isset($_POST['submit'])){
			...
			if (isset($_POST['keterangan'])) {
				if ($_POST['keterangan'] != '' or $_POST['keterangan'] != null) {
					$filters[] = "keterangan LIKE '%" . $_POST['keterangan'] . "%'";
					$r_keterangan = $_POST['keterangan'];
				}
			}
		...
		$data = $this->daftar_sbm_model->get($filters, $limit);
		//var_dump($data);
		$total_data = count($this->daftar_sbm_model->get($filters));
		$limit[] = $total_data;
	...
	public function insert(){
		...
		if(isset($_POST['submit'])){
			...
			if($_POST['keterangan'] == ''){
				$error_info[] = 'Keterangan can not be null';
				$error_status = true;
			}			
			...
			if($error_status == true){
				...
				$result = $this->daftar_sbm_model->insert($this->data['insert']);
				$info = array();
	...
	public function update(){
		...
		if(isset($_POST['submit'])){
			...
			if($error_status == true){
				...
				$result = $this->daftar_sbm_model->update($this->data['update'], $_POST['id']);
				if($result == true){
					$info = array();
			...
			$filter = array();
			$filter[] = "ID = ". $_POST['id'];
			$this->data['result'] = $this->daftar_sbm_model->get($filter);
			foreach($this->data['result'] as $value){
				$r_id 	= $value->ID;
				$r_kode = $value->KODE;
				$r_keterangan = $value->KETERANGAN;
			}
	...
	public function detail($id=null){
		...
		if(isset($_POST['id']) and $_POST['id'] != null){
			$filters = array();
			$filters[] = "ID = ". $_POST['id'];
			$data = $this->daftar_sbm_model->get($filters);
			...
			if (empty($data)) {
				...
				foreach($data as $value){
					$body[] = array(
						(object) array( 'classes' => ' bold align-left ', 'value' => 'Kode' ),
						(object) array( 'classes' => ' align-left ', 'value' => $value->KODE ),
					);					
					$body[] = array(
						(object) array( 'classes' => ' bold align-left ', 'value' => 'Keterangan' ),
						(object) array( 'classes' => ' align-left ', 'value' => $value->KETERANGAN ),
					);
					$body[] = array(
	...
	public function update_status(){
		...
		if(isset($_POST['id']) and $_POST['id'] != null){
			...
			$result = $this->daftar_sbm_model->get($filters);
			if($result != null){
	...
	public function delete(){
		...
		$result = $this->daftar_sbm_model->delete($this->data['delete']);
